...
const Marker = (() => {
  ...
  const SYSTEM = `You are an experienced NESA HSC Studies of Religion II marker at a marking centre.
...
- strengths / improvements: up to 3 short dot-point style phrases each`;
  ...
  function buildUserContent(q, qImg, student){
    ...
    content.push({ type: 'text', text:
      (q.year ? `QUESTION: ${q.year} HSC SOR II, Question ${q.n}${q.part ? '(' + q.part + ')' : ''} — worth ${q.marks} mark${q.marks === 1 ? '' : 's'}. Topic: ${q.topic}.`
              : `QUESTION: written for this course — worth ${q.marks} mark${q.marks === 1 ? '' : 's'}. Topic: ${q.topic}.`) +
