...
// This app shares its Firebase project with the HSC Health and Movement Science
// Quiz Arena. Every collection name gets an sor_ prefix so the two apps' data
// never collides in the one project.
const COL = n => 'sor_' + n;
...
const Store = (() => {
  ...
  // ---- roster ----
  // Who is on the roster is simply who has signed in. There is no separate list for
  // the teacher to keep in step, and — the point of doing it this way — no shared
  // document a student has to be able to write to in order to enrol. A student's own
  // record is the only thing their account can create, which is what lets the
  // security rules stay strict.
  ...
  async function saveRoster(names){
    ...
    if (cloud) {
      ...
      await db.collection(COL('meta')).doc('roster').set(patch, {merge:true});
    } else {
      lsSet('sorq_roster', clean);
      const rem = lsGet('sorq_removed', []).filter(n => !clean.includes(n));
      lsSet('sorq_removed', rem);
    }
    return clean;
  }
  async function removeFromRoster(name){
    ...
    if (cloud) await db.collection(COL('meta')).doc('roster').set({names: next, removed: firebase.firestore.FieldValue.arrayUnion(name)}, {merge:true});
    else {
      lsSet('sorq_roster', next);
      const rem = lsGet('sorq_removed', []); rem.push(name); lsSet('sorq_removed', rem);
    }
  }
  async function getRemoved(){
    if (cloud) {
      const doc = await db.collection(COL('meta')).doc('roster').get();
      return doc.exists ? (doc.data().removed || []) : [];
    }
    return lsGet('sorq_removed', []);
  }

  // ---- classes ----
  // Single source of truth: meta/roster.classes = { "Student Name": "SOR", ... }
  async function getClassMap(){
    if (cloud) {
      const doc = await db.collection(COL('meta')).doc('roster').get();
      return doc.exists ? (doc.data().classes || {}) : {};
    }
    return lsGet('sorq_classes', {});
  }
  async function setStudentClass(name, cls){
    if (cloud) {
      await db.collection(COL('meta')).doc('roster').set({classes: {[name]: cls || null}}, {merge:true});
    } else {
      const m = lsGet('sorq_classes', {});
      if (cls) m[name] = cls; else delete m[name];
      lsSet('sorq_classes', m);
    }
  ...
  async function setClassesBulk(map){
    if (cloud) await db.collection(COL('meta')).doc('roster').set({classes: map}, {merge:true});
    else { const m = {...lsGet('sorq_classes', {}), ...map}; lsSet('sorq_classes', m); }
  }
  function classList(){
    return (typeof CLASSES !== 'undefined' && CLASSES.length) ? CLASSES : ['SOR'];
  }

  // ---- settings ----
  // meta/settings holds anything the teacher can now change from the dashboard's
  // Settings tab instead of hand-editing config.js: the shared dashboard password,
  // the class names, and the XP/timer options. Both apps load this once at boot and
  // apply it on top of (never instead of) the config.js constants, so a site that has
  // never had its settings touched behaves exactly as config.js says.
  async function getSettings(){
    if (cloud) {
      const doc = await db.collection(COL('meta')).doc('settings').get();
      return doc.exists ? (doc.data() || {}) : {};
    }
    return lsGet('sorq_settings', {});
  }
  async function saveSettings(patch){
    if (cloud) { await db.collection(COL('meta')).doc('settings').set(patch, {merge:true}); return; }
    const cur = lsGet('sorq_settings', {});
    lsSet('sorq_settings', {...cur, ...patch});
  }
  // Applies any saved overrides onto the config.js constants in place, so every
  // existing call site (which reads CLASSES / QUIZ_OPTIONS / TEACHER_PASSWORD by
  // reference) picks the override up automatically without being rewritten.
  async function applySettings(){
    const s = await getSettings();
    if (s.classes && s.classes.length && typeof CLASSES !== 'undefined') {
      CLASSES.length = 0; CLASSES.push(...s.classes);
    }
    if (s.quizOptions && typeof QUIZ_OPTIONS !== 'undefined') Object.assign(QUIZ_OPTIONS, s.quizOptions);
    if (s.teacherPassword && typeof window !== 'undefined') window.TEACHER_PASSWORD_OVERRIDE = s.teacherPassword;
    return s;
  }
  ...
  async function getStudent(name){
    ...
    if (cloud) {
      const doc = await db.collection(COL('students')).doc(id).get();
      return doc.exists ? normaliseStudent(doc.data()) : null;
    }
    const all = lsGet('sorq_students', {});
    return all[id] ? normaliseStudent(all[id]) : null;
  }

  // ---- concurrent-write merge helpers for saveStudent ----
  // Every field merged here is monotonic (counts only grow, lists only gain
  // entries), so a union/max merge is safe and never throws away real progress.
  function unionArr(a, b, keyer){
    keyer = keyer || (x => x);
  ...
  function mergePairMap(a, b){
    // maps of key -> [correct, total]
    const out = {};
    for (const k of new Set([...Object.keys(a || {}), ...Object.keys(b || {})])) {
      const pa = (a && a[k]) || [0,0], pb = (b && b[k]) || [0,0];
      out[k] = [Math.max(pa[0] || 0, pb[0] || 0), Math.max(pa[1] || 0, pb[1] || 0)];
    }
    return out;
  }
  // Reconcile the doc we're about to write with whatever is already on the server,
  // so a teacher marking work while a student plays — or the same student open in
  // two tabs — can never have one write blindly clobber the other's XP/badges/attempts.
  function mergeStudentDocs(server, local){
    server = normaliseStudent({...server});
    local  = normaliseStudent({...local});
    const merged = {...server, ...local};
    merged.xp = Math.max(server.xp || 0, local.xp || 0);
    merged.bestStreak = Math.max(server.bestStreak || 0, local.bestStreak || 0);
    ...
    for (const [k, v] of Object.entries(local.lastSeen || {}))
      merged.lastSeen[k] = Math.max(merged.lastSeen[k] || 0, v || 0);
    // keep every attempt from both sides; a session written to only one copy
    // (e.g. mid-race) must not be dropped when the two are reconciled.
    const localDs = new Set((local.attempts || []).map(a => a.d));
    const extra = (server.attempts || []).filter(a => !localDs.has(a.d));
    merged.attempts = [...(local.attempts || []), ...extra].sort((a,b) => a.d - b.d);
    const t = merged.totals = {...(local.totals || {})};
    const st = server.totals || {};
    t.answered  = Math.max(st.answered || 0, t.answered || 0);
    t.correct   = Math.max(st.correct  || 0, t.correct  || 0);
    t.perTopic  = mergePairMap(st.perTopic,  t.perTopic);
    t.perSub    = mergePairMap(st.perSub,    t.perSub);
    t.perTopicQ = mergePairMap(st.perTopicQ, t.perTopicQ);
    return merged;
  }
  async function saveStudent(name, data){
    const id = slugify(name);
    if (cloud) {
      const ref = db.collection(COL('students')).doc(id);
      await db.runTransaction(async (tx) => {
        const doc = await tx.get(ref);
        const toWrite = doc.exists ? mergeStudentDocs(doc.data(), data) : data;
        tx.set(ref, toWrite);
      });
      return;
    }
    const all = lsGet('sorq_students', {});
    all[id] = data; lsSet('sorq_students', all);
  }

  async function deleteStudent(name){
    const id = slugify(name);
    if (cloud) { await db.collection(COL('students')).doc(id).delete(); return; }
    const all = lsGet('sorq_students', {});
    delete all[id]; lsSet('sorq_students', all);
  }
  ...
  async function createResponse(r){
    ...
    if (cloud) await db.collection(COL('responses')).doc(id).set(doc);
    else { const all = lsGet('sorq_responses', {}); all[id] = doc; lsSet('sorq_responses', all); }
    return doc;
  }
  // A response written by an older build - or one that lost a field somewhere -
  // used to take the whole marking queue down with it, because the dashboard
  // reads r.student on every row. Repair what can be repaired on the way in and
  // guarantee the shape of the rest, so a bad record is a bad ROW, never a blank
  // screen. Nothing is written back; this is display-side only.
  function normaliseResponse(r){
    if (!r || typeof r !== 'object') return r;
    // older/hand-edited docs stored the author under a different key
    if (r.student == null || r.student === '')
      r.student = r.name || r.studentName || r.by || null;
    if (r.status == null) r.status = (r.mark == null ? 'pending' : 'marked');
    if (!r.peers || typeof r.peers !== 'object') r.peers = {};
    if (!Array.isArray(r.subs)) r.subs = [];
    const max = Number(r.max);
    r.max = (Number.isFinite(max) && max > 0) ? Math.round(max) : 0;   // 0 = unknown; callers fall back to the question
    if (typeof r.typed !== 'string') r.typed = r.typed == null ? '' : String(r.typed);
    if (!r.label) r.label = [r.year, r.qn ? 'Q' + r.qn : null].filter(Boolean).join(' ') || r.qid || r.id || 'Untitled';
    if (!r.topic) r.topic = '—';
    // A record missing this (hand-edited, or written by an older build) used to sort
    // as NaN, which shuffles the marking queue into an arbitrary order every render.
    if (typeof r.submitted !== 'number') r.submitted = r.markedAt || 0;
    return r;
  }

  async function getAllResponses(){
    if (cloud) { const snap = await db.collection(COL('responses')).get(); return snap.docs.map(d => normaliseResponse(d.data())); }
    return Object.values(lsGet('sorq_responses', {})).map(normaliseResponse);
  }
  ...
  async function setResponseAI(id, ai){
    if (cloud) { await db.collection(COL('responses')).doc(id).set({ai}, {merge:true}); return; }
    const all = lsGet('sorq_responses', {});
    if (all[id]) { all[id].ai = ai; lsSet('sorq_responses', all); }
  }
  async function setResponseMark(id, mark, feedback){
    const patch = {mark, feedback: feedback || '', status: 'marked', markedAt: Date.now()};
    if (cloud) { await db.collection(COL('responses')).doc(id).set(patch, {merge:true}); }
    else { const all = lsGet('sorq_responses', {}); if (all[id]) { Object.assign(all[id], patch); lsSet('sorq_responses', all); } }
  }
  // Send a marked response back to the queue so the teacher can change the mark.
  async function reopenResponse(id){
    if (cloud) { await db.collection(COL('responses')).doc(id).set({status: 'pending'}, {merge:true}); return; }
    const all = lsGet('sorq_responses', {});
    if (all[id]) { all[id].status = 'pending'; lsSet('sorq_responses', all); }
  }
  async function addPeerMark(id, name, mark, xp){
    const key = slugify(name);
    const entry = {mark, d: Date.now(), xp};
    if (cloud) await db.collection(COL('responses')).doc(id).set({peers: {[key]: entry}}, {merge:true});
    else {
      const all = lsGet('sorq_responses', {});
      if (all[id]) { all[id].peers = all[id].peers || {}; all[id].peers[key] = entry; lsSet('sorq_responses', all); }
    }
  }
  async function deleteResponse(id){
    if (cloud) { await db.collection(COL('responses')).doc(id).delete(); return; }
    const all = lsGet('sorq_responses', {}); delete all[id]; lsSet('sorq_responses', all);
  }
  // Used when a student's results are wiped, so their submitted answers don't linger
  // in the marking queue referencing a student record that no longer exists.
  async function deleteResponsesFor(name){
    const mine = await getResponsesFor(name);
    await Promise.all(mine.map(r => deleteResponse(r.id)));
  }
  ...
  async function createDuel(from, to, qids){
    ...
    if (cloud) await db.collection(COL('duels')).doc(id).set(duel);
    else { const all = lsGet('sorq_duels', {}); all[id] = duel; lsSet('sorq_duels', all); }
    return duel;
  }
  async function getDuelsFor(name){
    if (cloud) {
      const [a, b] = await Promise.all([
        db.collection(COL('duels')).where('from','==',name).get(),
        db.collection(COL('duels')).where('to','==',name).get()
      ]);
      const seen = {};
      [...a.docs, ...b.docs].forEach(d => { seen[d.id] = d.data(); });
      return Object.values(seen).sort((x,y) => y.created - x.created);
    }
    return Object.values(lsGet('sorq_duels', {}))
  ...
  async function saveDuelResult(id, name, result){
    const key = slugify(name);
    if (cloud) {
      await db.collection(COL('duels')).doc(id).set({res: {[key]: result}}, {merge:true});
      const doc = await db.collection(COL('duels')).doc(id).get();
      return doc.exists ? doc.data() : null;
    }
    const all = lsGet('sorq_duels', {});
    if (!all[id]) return null;
    all[id].res = all[id].res || {};
    all[id].res[key] = result;
    lsSet('sorq_duels', all);
    return all[id];
  }
  async function deleteDuel(id){
    if (cloud) { await db.collection(COL('duels')).doc(id).delete(); return; }
    const all = lsGet('sorq_duels', {}); delete all[id]; lsSet('sorq_duels', all);
  }
  async function getAllDuels(){
    if (cloud) { const snap = await db.collection(COL('duels')).get(); return snap.docs.map(d => d.data()); }
    return Object.values(lsGet('sorq_duels', {}));
  }
  ...
  return { init, isCloud: () => cloud, getRoster, saveRoster, removeFromRoster, getRemoved,
           ...
           setResponseAI, setResponseMark, reopenResponse, addPeerMark, deleteResponse, deleteResponsesFor };
})();
...
function levelName(lv){
  const names = ["Bench Warmer","Rookie","Club Player","First Grader","Team Captain","State Athlete","National Squad","Pro Athlete","Olympian","Hall of Famer"];
  return names[Math.min(lv-1, names.length-1)] + (lv > names.length ? " " + (lv - names.length + 1) : "");
}

// Names/emoji for the per-topic mastery badges (below). Keyed by SYLLABUS topic
// name; a topic missing here still gets a badge with a generic name/emoji.
const TOPIC_BADGE_META = {
  'Australia post-1945': {emoji:'🪃', name:'Australia MVP'},
  'Christianity': {emoji:'⛪', name:'Christianity MVP'},
  'Islam': {emoji:'🕌', name:'Islam MVP'},
  'Religion and Peace': {emoji:'🕊️', name:'Peace MVP'},
  'Religion and Non-Religion': {emoji:'🧭', name:'Comparative MVP'}
};
function topicMasterBadgeId(topic){
  return 'master_' + topic.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-+|-+$)/g,'');
}

const BADGES = [
  {id:'first_steps', emoji:'🎯', name:'Rookie Debut',  desc:'Complete your first quiz'},
  {id:'full_section',emoji:'💪', name:'Full Match',    desc:'Complete a 20-question quiz'},
  {id:'sharpshooter',emoji:'🏹', name:'Sharpshooter',  desc:'Score a perfect game on a quiz of 10+ questions'},
  {id:'streak_5',    emoji:'🔥', name:'Hot Streak',    desc:'Answer 5 in a row correctly'},
  ...
  {id:'explorer',    emoji:'🗺️', name:'Scout',         desc:'See every question in the bank'},
  // Per-topic mastery badges, generated from SYLLABUS so any topic set works.
  ...Object.keys(SYLLABUS).map(topic => ({
    id: topicMasterBadgeId(topic),
    emoji: (TOPIC_BADGE_META[topic] && TOPIC_BADGE_META[topic].emoji) || '🎓',
    name: (TOPIC_BADGE_META[topic] && TOPIC_BADGE_META[topic].name) || (topic + ' Scholar'),
    desc: `85%+ accuracy over 25+ ${topic} questions`
  })),
  ...
  {id:'streak_15',   emoji:'🌟', name:'Legendary Run', desc:'Answer 15 in a row correctly'},
  {id:'dedication',  emoji:'📅', name:'Team Regular',  desc:'Complete 10 quizzes'},
  ...
  {id:'perfectionist',emoji:'👑', name:'Undefeated',   desc:'Score 100% on a 20-question quiz'},
  {id:'clean_slate', emoji:'🧼', name:'Clean Sheet',   desc:'Clear every question from My Mistakes'},
  ...
  {id:'sa_first',    emoji:'📝', name:'Match Report',  desc:'Complete your first short-answer session'},
  {id:'sa_perfect',  emoji:'✒️', name:'Top Band',      desc:'Full marks on a 4+ mark short answer'},
  {id:'sa_20',       emoji:'🖋️', name:'Playmaker',     desc:'Have 20 short answers marked'},
  // --- game modes ---
  {id:'daily_first', emoji:'📆', name:'Game Day One',  desc:'Complete your first Daily Challenge'},
  {id:'daily_perfect',emoji:'🌞', name:'Flawless Daily',desc:'Get 5/5 on a Daily Challenge'},
  {id:'daily_streak5',emoji:'🗓️', name:'Regular Season',desc:'Daily Challenge 5 days in a row'},
  ...
  {id:'exam_first',  emoji:'📄', name:'Game Day',      desc:'Complete an Exam Simulation'},
  ...
  // --- syllabus drills ---
  {id:'syl_first',   emoji:'🧠', name:'Rookie Trainer', desc:'Complete your first Syllabus Drill'},
...
function evaluateBadges(student, attempt, totalQuestions){
  ...
  const checks = {
    ...
    ...Object.fromEntries(Object.keys(SYLLABUS).map(topic =>
      [topicMasterBadgeId(topic), () => topicAcc(topic) >= 0.85])),
    ...
    all_rounder:() => Object.keys(SYLLABUS)
